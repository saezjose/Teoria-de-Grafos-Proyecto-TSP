...
class AppTSP(ctk.CTk):
    def __init__(self):
        ...
        # Bandera para controlar el cierre seguro
    ...
    def on_closing(self):
        ...
        if hasattr(self, 'mapa') and self.mapa:
            try:
                self.mapa.destroy() # Asumiendo que MapaGrafico tiene un método destroy o limpiamos el widget
            except Exception:
                pass
            
        # IMPORTANTE: self.quit() detiene el mainloop y evita los errores de "invalid command name"
        # relacionados con check_dpi_scaling y updates pendientes.
        self.quit()
        self.destroy()
    ...
    def animar(self):
        ...
        try:
            ...
            # Verificar de nuevo antes de dibujar
            if self.is_closing: return
            ...
            # Agendar siguiente frame solo si no estamos cerrando
            if not self.is_closing:
